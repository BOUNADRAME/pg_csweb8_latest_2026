...
Commande CLI pour effectuer le **breakout sélectif** des questionnaires CSPro vers la base de données analytique (PostgreSQL/MySQL/SQL Server).

<Callout type="info">
**Innovation** : Cette commande implémente le breakout sélectif par dictionnaire développé par **Assietou DIAGNE** (ANSD, Sénégal), validé en production sur le **RGPH5** avec 3.2M questionnaires.
</Callout>
...
Le nom du dictionnaire doit correspondre **exactement** au nom dans CSWeb (case-sensitive).
</Callout>
...
Nombre de threads parallèles pour le traitement des questionnaires.
...
<Callout type="error">
**Attention :** `--overwrite` **supprime toutes les données** des tables existantes. Faire un backup avant !
</Callout>
...
<Callout>
**Convention :** Les noms de tables sont toujours en **minuscules** pour compatibilité PostgreSQL.
</Callout>

## Tables Créées

Pour chaque dictionnaire, les tables suivantes sont automatiquement créées :
...
Tables pour chaque niveau hiérarchique (ménage, individu, observation, etc.).
...
Tables pour chaque record CSPro défini dans le dictionnaire.
...
```bash
# Via MySQL metadata
docker-compose exec csweb-mysql mysql -u csweb -p csweb
