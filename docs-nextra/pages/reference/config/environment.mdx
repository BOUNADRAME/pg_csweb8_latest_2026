...
Toutes les configurations de CSWeb Community Platform sont définies dans le fichier **`.env`** situé à la racine du projet.

<Callout type="warning">
**IMPORTANT :**
- ✅ Copier `.env.example` vers `.env` lors de l'installation
- ✅ Modifier les valeurs selon votre environnement
- ❌ **NE JAMAIS** committer `.env` (contient mots de passe)
...
<Callout>
**Note :** Cette variable est **informative** uniquement. Le choix du driver se fait via `BREAKOUT_DB_TYPE`.
</Callout>

### BREAKOUT_DB_TYPE

Type de base de données pour le breakout sélectif.
...
<Callout type="error">
**Attention :** Si vous changez `BREAKOUT_DB_TYPE`, vous devez **redémarrer les containers** :
```bash
docker-compose down
docker-compose --profile local-<TYPE> up -d
```
</Callout>
...
Active le mode debug (affichage erreurs détaillées).
...
<Callout type="warning">
**JAMAIS** activer debug en production (risque de fuite d'informations sensibles).
</Callout>
...
Fuseau horaire de l'application (format PHP timezone).
...
Clé secrète pour chiffrement/hachage (Symfony).
...
<Callout type="error">
**CRITIQUE :**
- ✅ Générer une clé **unique** par environnement
- ✅ Ne **JAMAIS** partager cette clé
- ❌ Ne **JAMAIS** utiliser la valeur d'exemple
- ❌ Ne **JAMAIS** committer cette valeur
</Callout>

### CSWEB_PORT

Port HTTP pour accéder à CSWeb.
...
<Callout type="info">
**À SAVOIR :**
- Base de données **imposée** par CSWeb vanilla (schéma non modifiable)
- Contient : users, dictionaries, oauth_clients, sync_queue, system_config
- **TOUJOURS** sur la même machine que CSWeb (localhost ou container `mysql`)
- **NE JAMAIS** utiliser cette base pour le breakout (performances limitées)
</Callout>

### MYSQL_HOST

Nom d'hôte du serveur MySQL metadata.
...
Port du serveur MySQL metadata.
...
Nom de la base de données metadata.
...
<Callout>
**Note :** Cette base est créée automatiquement par `setup.php` lors de l'installation.
</Callout>

### MYSQL_USER

Utilisateur MySQL pour la base metadata.
...
Mot de passe de l'utilisateur MySQL metadata.
...
<Callout type="error">
**CRITIQUE :**
- ✅ Mot de passe **très fort** (16+ caractères)
- ✅ Différent de `MYSQL_PASSWORD`
- ❌ Ne **JAMAIS** utiliser en production depuis applications
- ❌ Uniquement pour admin/maintenance
</Callout>
...
Variables utilisées si `BREAKOUT_DB_TYPE=postgresql`.
...
Nom de la base de données breakout PostgreSQL.
...
Utilisateur PostgreSQL pour la base breakout.
...
Variables utilisées si `BREAKOUT_DB_TYPE=mysql`.

### MYSQL_BREAKOUT_HOST

Nom d'hôte du serveur MySQL breakout.
...
Port du serveur MySQL breakout.
...
### MYSQL_BREAKOUT_DATABASE

Nom de la base de données breakout MySQL.
...
Utilisateur MySQL pour la base breakout.
...
Mot de passe de l'utilisateur MySQL breakout.
...
Variables utilisées si `BREAKOUT_DB_TYPE=sqlserver`.

### SQLSERVER_HOST

Nom d'hôte du serveur SQL Server.
...
Nom de la base de données breakout SQL Server.
...
- Noms avec PascalCase (`CSWeb_Analytics`)
- Noms de tables avec underscore (`eval_producteurs`)
</Callout>
...
<Callout type="warning">
**Production :** Créer un utilisateur dédié avec permissions limitées (pas `sa`).
</Callout>
...
```
**Solution :** Utiliser majuscule + chiffre + symbole (ex: `MyPass123!`)
</Callout>
...
Port pour accéder à phpMyAdmin (interface MySQL).
...
Port pour accéder à pgAdmin (interface PostgreSQL).
...
<Callout type="warning">
**Production :** Désactiver phpMyAdmin et pgAdmin (risque sécurité). Utiliser CLI ou clients locaux.
</Callout>
...
Clé secrète pour signer les tokens JWT OAuth2.
...
<Callout type="error">
**CRITIQUE :**
- ✅ Différent de `APP_SECRET`
- ✅ Unique par environnement
- ❌ Ne **JAMAIS** changer en production (invaliderait tous les tokens)
</Callout>
...
Dossier de stockage des fichiers uploadés (photos, documents).
...
<Callout>
**Production :**
- Utiliser un volume Docker persistant
- Sauvegarder régulièrement ce dossier
- Vérifier permissions (www-data:www-data)
</Callout>

### API_URL

URL de base de l'API CSWeb (utilisée pour webhooks, callbacks).
...
<Callout type="warning">
**Attention :** Terminer par `/` (slash final obligatoire).
</Callout>
...
Niveau de logs de l'application CSWeb.
...
### CSWEB_PROCESS_CASES_LOG_LEVEL

Niveau de logs spécifique à la commande `process-cases-by-dict`.
...
Temps maximum d'exécution PHP en secondes.
...
<Callout type="warning">
**Breakout :** Pour `process-cases-by-dict` avec millions de cas, augmenter ou désactiver (0 = illimité).
</Callout>
...
CSWEB_LOG_LEVEL=error
CSWEB_PROCESS_CASES_LOG_LEVEL=error
MAX_EXECUTION_TIME=0  # Illimité pour breakout 3.2M cas
```
...
✅ MySQL Metadata : Connected to mysql:3306/csweb_metadata
✅ PostgreSQL Breakout : Connected to postgres:5432/csweb_analytics
```
...
- **Architecture Flexible** : [Architecture - Flexible](/guides/architecture/flexible)
