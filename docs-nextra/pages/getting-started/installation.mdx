...
2. **Page Configuration** (`/setup/configure.php`) :
   ...
   | **CSWeb API URL**           | `http://localhost:8080/api/` | **Port 8080 accessible depuis le navigateur**                      |

   > **Important :** L'API URL doit être `http://localhost:8080/api/` (avec le port 8080).
   > C'est l'URL accessible depuis votre navigateur pour accéder à l'API CSWeb.
   > Le port 8080 est le port externe mappé dans Docker qui redirige vers le port 80 interne du conteneur.
