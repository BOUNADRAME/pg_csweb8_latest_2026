...
Transformer **CSWeb vanilla (MySQL only)** en une version supportant **PostgreSQL** pour permettre :
...
Ce travail technique a été réalisé par **Assietou DIAGNE** (ANSD, Sénégal) dans le cadre du projet RGPH5 (Recensement Général de la Population et de l'Habitat du Sénégal).

<Callout type="success">
  **Impact :** Cette transformation a permis à l'ANSD de breaker des millions de questionnaires RGPH5 vers PostgreSQL pour analyse avec Power BI et Tableau.
</Callout>
...
❌ Support MySQL **uniquement** pour le breakout
❌ Pas de driver PostgreSQL intégré
...
class DictionarySchemaHelper
{
    ...
    public function __construct($dbType = 'mysql')
    {
        ...
        switch ($dbType) {
            case 'pgsql':
                $this->connection = new PDO('pgsql:host=...'); // ✅ PostgreSQL
                break;
            case 'mysql':
...
<Callout type="info">
  **Clé :** La variable `$dbType` permet de choisir dynamiquement le SGBD au lieu de hardcoder MySQL.
</Callout>
...
private function cleanDictionarySchema() {
    try {
        ...
        if ((is_countable($tables) ? count($tables) : 0) > 0) {
            $this->conn->prepare("SET FOREIGN_KEY_CHECKS = 0;")->execute(); // MySQL specific
...
private function createDictionarySchema($processCasesOptions) {
    ...
    try {
        ...
        // ✅ PostgreSQL: Exécuter chaque statement séparément
...
public function generateDictionary(Dictionary $dictionary, $processCasesOptions) {
    ...
    //TODO check for Charset | Collation | comment
    for ($iLevel = 0; $iLevel < (is_countable($dictionary->getLevels()) ?
...
<Callout type="warning">
  **Attention :** Cette section contient le code complet de transformation des tables. C'est le **cœur technique** du travail d'Assietou.
</Callout>
...
<Callout type="warning">
  **Pour Développeurs Avancés Seulement**

  Cette section est destinée aux développeurs qui veulent reproduire la transformation PostgreSQL sur un CSWeb vanilla **sans configuration préexistante**.
</Callout>
...
Le code complet de transformation est disponible dans le repository CSWeb Community v2.0 :
...
Pour toute question technique sur la transformation PostgreSQL :
...
Vous avez amélioré le code ou trouvé des bugs ?
...
👉 [Repository GitHub](https://github.com/BOUNADRAME/pg_csweb8_latest_2026)
...
- [Architecture Flexible Local/Remote →](/guides/architecture/flexible)
