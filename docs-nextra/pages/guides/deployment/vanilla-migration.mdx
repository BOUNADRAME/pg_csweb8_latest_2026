# Migration depuis CSWeb Vanilla

Guide de migration de **CSWeb vanilla** vers **CSWeb Community Platform v2.0**.
...
<Callout type="info">
**Base officielle :** CSWeb Community Platform est basé sur **CSWeb 8** téléchargé depuis [csprousers.org/downloads](https://csprousers.org/downloads/). Migration depuis CSWeb vanilla (toutes versions) est supportée.
</Callout>
...
sleep 10  # Attendre démarrage MySQL

docker-compose exec -T mysql mysql -u root -p csweb_metadata < /tmp/backup_vanilla_metadata.sql

# Vérifier
docker-compose exec mysql mysql -u root -p csweb_metadata

SHOW TABLES;  # Doit afficher tables CSPro
...
SELECT name, label FROM cspro_dictionaries;
