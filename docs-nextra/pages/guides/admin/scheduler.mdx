...
| Toggle on/off | Commenter/décommenter | Bouton play/pause |
...
3. Le card **Breakout Scheduler** apparaît sous le card Data Configuration
...
| **Last Result** | Badge OK (vert) ou Error avec code (rouge) |
| **Actions** | Toggle, Edit, Delete |

### Actions disponibles

- **Toggle** (play/pause) : Active ou désactive le schedule sans le supprimer
- **Edit** (crayon) : Modifier l'expression cron ou le statut enabled
- **Delete** (corbeille) : Supprimer le schedule (avec confirmation)
...
4. **Monitoring** : Consulter régulièrement la colonne "Last Result" pour détecter les erreurs
