...
class DictionarySchemaHelper {
    ...
    private function getConnectionParameters(): bool {
        $stm = "SELECT host_name, port, db_type, schema_name, schema_user_name, AES_DECRYPT(schema_password, '" . "cspro') as `password` FROM `cspro_dictionaries_schema` JOIN `cspro_dictionaries` ON dictionary_id = cspro_dictionaries.id WHERE cspro_dictionaries.dictionary_name = :dictName";
